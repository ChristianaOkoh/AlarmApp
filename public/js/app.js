...
const saveReminderToDatabase = async (title, description, dateTimeString) => {
  ...
  try {
    const response = await fetch("https://purring-vanya-testalarmapp-513052a3.koyeb.app/api/reminders", {
      method: "POST",
      headers: {
...
const deleteReminder = async (reminderId, row) => {
  try {
    const response = await fetch(`https://purring-vanya-testalarmapp-513052a3.koyeb.app/api/reminders/${reminderId}`, {
      method: 'DELETE',
    });
    if (response.ok) {
